...
class SqList{
    ...
    template<class ElemType>
    void ListCreate( SqList<ElemType> &sqlist, vector<ElemType> &A ){
         // 本算法生成顺序表
        int len = A.size();
        for(int i = 0; i < len; i++){         
    ...
    template<class ElemType>
    void CreateList( stringstream &input, SqList<ElemType> &L){
    ...
    void Intersect_Sq_OL_C( const SqList<ElemType> &A, const SqList<ElemType> &B, SqList<ElemType> &C  ){
         ...
         while( i <= A.ListLength() && j <= B.ListLength() ){
              if( A.GetElem(i) < B.GetElem(j) ) i++;
              else if( A.GetElem(i) > B.GetElem(j) ) j++;
              else if( A.GetElem(i) == B.GetElem(j) ){
                  k++;
                  C.ListInsert(k, A.GetElem(i) );
                  i++;
                  j++;
              }
         }     
    ...
    int main(){
        ...
        getline(cin,temp1);
        stringstream input1(temp1);
    
        getline(cin,temp2);
        stringstream input2(temp2);
    
        if(n==0){
            //建立顺序表 
            SqList <int> sqlist1, sqlist2, sqlist3;
            CreateList(input1, sqlist1);
            CreateList(input2, sqlist2);
            //应用3：删除顺序表sqlist2中多余的数据元素，即使操作之后的顺序表中所有元素的值都不相同 
            Intersect_Sq_OL_C(sqlist1, sqlist2, sqlist3);
            cout<<endl;
            sqlist3.ListTraverse();
        }     
        else if(n==1){
            //建立顺序表 
            SqList <double> sqlist1, sqlist2, sqlist3;
            CreateList(input1, sqlist1);
            CreateList(input2, sqlist2);
            //应用3：删除顺序表sqlist2中多余的数据元素，即使操作之后的顺序表中所有元素的值都不相同 
            Intersect_Sq_OL_C(sqlist1, sqlist2, sqlist3);
            ...
            //建立顺序表 
            SqList <char> sqlist1, sqlist2, sqlist3;
            CreateList(input1, sqlist1);
            CreateList(input2, sqlist2);
            //应用3：删除顺序表sqlist2中多余的数据元素，即使操作之后的顺序表中所有元素的值都不相同 
            Intersect_Sq_OL_C(sqlist1, sqlist2, sqlist3);
            ...
            //建立顺序表 
            SqList <string> sqlist1, sqlist2, sqlist3;
            CreateList(input1, sqlist1);
            CreateList(input2, sqlist2);
            //应用3：删除顺序表sqlist2中多余的数据元素，即使操作之后的顺序表中所有元素的值都不相同 
            Intersect_Sq_OL_C(sqlist1, sqlist2, sqlist3);
